...
def farfeild_txt_to_np(txt_file_path:str):
    ...
    f = open(r"D:\python files\Efficiency.pickle", 'rb')
    [a,b,c] = pickle.load(f)
    f.close()
    # parse the .txt file
    ...
    farfeild = np.zeros((37, 72, 6))  # 73 rows for Theta (0 to 180 with 2.5 increments), 144 columns for Phi (0 to 357.5 with 2.5 increments), 6 channels
    ...
    for i, (theta, phi) in enumerate(theta_phi):
        ...
        theta_index = int(theta / 5)  # Scale theta to match tensor indices
        phi_index = int(phi / 5)  # Scale phi to match tensor indices
    ...
    # get directivity
    radiation_pattern = farfeild[:,:,0]
    radiation_pattern[radiation_pattern<1e-3] = 1e-3
    # farfeild = np.stack((theta_abs, phi_abs, theta_phase, phi_phase), axis=-1)
    import matplotlib
    matplotlib.use('Qt5Agg')
    import matplotlib.pyplot as plt
    plt.subplot(1, 1, 1)
    plt.imshow(10*np.log10(radiation_pattern),extent=[0,360,0,180],cmap='jet')  # , cmap='gray')
...
farfeild_txt_to_np(r"D:\python files\farfield (f=2400) [1].txt")
# import pickle
