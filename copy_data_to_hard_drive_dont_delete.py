...
destination_path = r'C:\Users\snirnehemia\OneDrive - Tel-Aviv University\Documents\parametric results\output'
...
start = int(100.144e3)#min(result_list)
...
for ID in range(start, stop):
    ...
    if (os.path.isfile(os.path.join(origin_path,s_image)) and
        ...
        copy_tree(os.path.join(origin_path,model_folder), os.path.join(destination_path,model_folder))
        shutil.copy(os.path.join(origin_path,model_image), os.path.join(destination_path,model_image))
        copy_tree(os.path.join(origin_path,result_folder), os.path.join(destination_path,result_folder))
